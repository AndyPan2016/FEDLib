...
var a = function(name, options, age){
	...
	age = age || '0';
	options = options || {};
	console.log(name+':'+age+':'+options.haha);
};

a.callee({age: '21', options: {haha: 'haha'}}, 'this');

var b = {
	c: function(options, name, age){
		name = name || '...';
		age = age || '0';
		options = options || {};
		console.log(name+':'+age+':'+options.haha);
...
b.c.callee({age: '21', options: {haha: 'haha'}});
*/

/*
var D = function(){
	var tempObj = {};
	this.info = function(options, name, age){
		name = name || '...';
		age = age || '0';
		options = options || {};
		console.log(name+':'+age+':'+options.haha);
...
var d = new D();
d.info.callee({name: '张三', age: '21', options: {haha: 'haha'}});
d.set('default', 'default');
d.set.callee('callee', 'callee');
d.set.callee({key: 'a', val: 'a'});
d.set.kvpcallee({b: 'b'});
d.set.kvpcallee({c: 'c', d: 'd'});
d.set.kvpcallee([{e: 'e'}, {f: 'f'}]);
d.get();
*/

/*var a = {
	b: function(args, test){
		console.log('run a.b;'+args+test);
	},
	c: function(){
		console.log('run a.c;');
	}
}.listen().on('run', function(){
	console.log('run event:--->all');
});

a.b.bind('run', function(args){
	console.log('run event:--->'+args);
});

a.b('this is a.b', 'test');
a.b({args: 'this is a.b', test: 'test'});

a.c();*/

/*var MyClass = function(){
	var _self = this;
	_self.say = function(a, b){
		console.info('say--->'+a+b);
	};

	_self.listen().on('run', function(a, b){
		console.log('run listen---args:'+a+b);
	});
};
var clas = new MyClass();
clas.say('a', 'b');
//clas.say({a: 'a', b: 'b'});
//clas.say.call('this', {a: 'a', b: 'b'});*/
...
x.on('run', function(a, b, c){
	console.log('run event x--->'+a+b+c);
}).on('finish', function(a, b, c){
	console.log('finish event x--->'+a+b+c);
});
